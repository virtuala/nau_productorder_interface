...
public class UserroleController extends ComPageSample {
    /* 日志方法 */
    protected final Logger sLogger = LoggerFactory.getLogger(this.getClass());

    /* 服务 */
    @Autowired
    private UserroleService sService;


    @GetMapping("/list")
    public ResultCode OnList(Userrole tmpPojo,Userinfo pAuth) {

        List<Userrole> tmpList= this.sService.getFindAll();
    ...
    public ResultCode OnList(String ids, Userinfo pAuth) {
        ...
        Userrole tmpPojo= this.sService.OnNormal(ids);
        if(tmpPojo==null){
            tmpPojo=new Userrole();
        }

        List<Userrole> tmpListObject=new ArrayList<Userrole>();
    ...
    @PostMapping("/add")
    public ResultCode OnAdd(Userrole tmpPojo,Userinfo pAuth) {
    ...
    @PostMapping("/modify")
    public ResultCode OnModify(String ids,Userrole tmpPojo,Userinfo pAuth) {
