...
import com.nets.productorder.orderinfo.service.Orderinfo.OrderinfodetailService;
...
public class OrderinfodetailController extends ComPageSample {
    /* 日志方法 */
    protected final Logger sLogger = LoggerFactory.getLogger(this.getClass());

    /* 服务 */
    @Autowired
    private OrderinfodetailService sService;


    @GetMapping("/list")
    public ResultCode OnList(Orderinfodetail tmpPojo, Userinfo pAuth) {

        List<Orderinfodetail> tmpList= this.sService.getFindAll();
    ...
    public ResultCode OnList(String ids) {
        ...
        Orderinfodetail tmpPojo= this.sService.OnNormal(ids);
        if(tmpPojo==null){
            tmpPojo=new Orderinfodetail();
        }

        List<Orderinfodetail> tmpListObject=new ArrayList<Orderinfodetail>();
    ...
    @PostMapping("/add")
    public ResultCode OnAdd(Orderinfodetail tmpPojo) {
    ...
    @PostMapping("/modify")
    public ResultCode OnModify(String ids,Orderinfodetail tmpPojo) {
